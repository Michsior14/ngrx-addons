...
const isRehydrateAction = (action: Action): action is ReturnType<typeof rehydrate> => action.type === REHYDRATE;
  ...
  (state: T, action: V): T => {
    ...
    if (isRehydrateAction(action)) {
      ...
      Object.keys(features).forEach((key) => {
        newState[key as keyof T] = { ...state[key as keyof T], ...(features[key] as T[keyof T]) };
      });
    }
